  ...
  async updateNotificationPreferences(
    ...
    try {
      ...
      if (fcmToken !== undefined && fcmToken !== null) {
        ...
        await conn.execute(
          `UPDATE users SET notification_category_preferences = ?, fcm_token = ?, updated_at = CURRENT_TIMESTAMP(3)
           WHERE id = ?`,
          [prefsJson, fcmToken, userId]
        );
      } else if (fcmToken === null) {
        await conn.execute(
          `UPDATE users SET notification_category_preferences = ?, fcm_token = NULL, updated_at = CURRENT_TIMESTAMP(3)
           WHERE id = ?`,
          [prefsJson, userId]
        );
      } else {
        await conn.execute(
          `UPDATE users SET notification_category_preferences = ?, updated_at = CURRENT_TIMESTAMP(3)
           WHERE id = ?`,
          [prefsJson, userId]
        );
      }
